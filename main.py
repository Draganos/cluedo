...
class Game:
        ...
        for dx, dy in possible_moves:
            ...
            if (new_col, new_row) in self.doors or (
                ...
                if new_dist < current_dist:
                    return (dx, dy)
        valid_moves = [] # changed to make sure moves are valid
        for dx, dy in possible_moves:
            new_col, new_row = p.col + dx, p.row + dy
    ...
    def end_turn(self):
        ...
        self.turn_index += 1
        if self.turn_index >= len(self.all_players):         #reset to 0 if we go past the last player
            self.turn_index = 0
        ...
        while self.all_players[self.turn_index].is_eliminated:
            ...
            if self.turn_index >= len(self.all_players):
                self.turn_index = 0
        #self.turn_index = (self.turn_index + 1) % len(self.all_players)   <--- removed as this can cause turns to skip players
        self.moves_left = 0
